    ...
    export default {
        ...
        methods: {
            ...
            changeNumber: function (ele) {
                ...
                let level = parseInt(this.$route.params.level);
                let activeIndex = ele+1; // 下标数字
                let activeNumber = this.levelArray[ele]; // 下标内容数字
                // 点击空格以外的数字事件
                
                if (activeNumber != level*level) {
                    // 点击数字获取当前一列的数组
                    let rolArr = this.levelArray.filter((ele,index)=>{
                            return (index+1) % level === activeIndex % level;
                        })
                            // console.log(rolArr);
                            // console.log(activeNumber);

                    // 设置当前点击值于空格值比较，判断空格在正轴还是在负轴
                    let rolActiveNum;
                    let rolLevelNum;
                    let rolArr2 = rolArr.forEach((ele,index)=>{
                        if(activeNumber == ele){
                            rolActiveNum = index + 1;
                        }
                        if(level * level == ele){
                            rolLevelNum = index + 1;
                        }
                        if(rolActiveNum - rolLevelNum < 0){
                            console.log('正轴');
                            console.log(rolArr);
                            rolArr.splice(rolActiveNum-1,0,level*level);
